...
import { FormControl, FormLabel, RadioGroup, FormControlLabel, Radio, colors } from '@mui/material';
...
const handleFilter=(value,sectionId)=>{
  ...
  // If the value is already present in the params, remove it
  if (filterValues.includes(value)) {
    filterValues = filterValues.filter((item) => item !== value);
    // If no values are left, remove the sectionId from the params
                  ...
                  <Disclosure as="div" key={section.id} className="border-t border-gray-200 px-4 py-6">
                    {({ open }) => (
                      <>
                        <h3 className="-mx-2 -my-3 flow-root ">
                          <DisclosureButton className="flex w-full items-center justify-between bg-white px-2 py-3 text-gray-400 hover:text-gray-500">
                            <FormLabel sx={{color:"black"}}className="font-medium text-gray-900">{section.name}</FormLabel>
                            <span className="ml-6 flex items-center">
                              {open ? (
                                <MinusIcon className="h-5 w-5" aria-hidden="true" />
                              ) : (
                        ...
                        <DisclosurePanel className="text-left">
                          <FormControl>
                            <RadioGroup
